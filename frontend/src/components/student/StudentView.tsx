...
export const StudentView: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-61px)] bg-[#0e0e11] text-[#e3e3e3] overflow-hidden">
      ...
      {/* Horizontal Scrollable Meeting Filter Chips */}
      <div className="px-4 md:px-8 py-2 bg-[#131314]/40 border-b border-[#1e1f20]/60 flex items-center gap-2 overflow-x-auto no-scrollbar">
        <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1 flex-shrink-0 mr-1">
          <Filter className="w-3 h-3 text-sky-400" /> Filter Materi:
        </span>

        <button
          onClick={clearMeetingFilters}
          className={`px-3 py-1 rounded-full text-xs font-semibold transition flex-shrink-0 flex items-center gap-1 ${
        ...
        {meetings.map((m) => {
          ...
          return (
            <button
              ...
              className={`px-3 py-1 rounded-full text-xs font-medium transition flex-shrink-0 flex items-center gap-1.5 ${
                isSelected
